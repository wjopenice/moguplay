...
class IndexController extends BaseController {
    ...
    public function index(){
        ...
        $this->rank();
        //$this->zixun_adv();
        $this->assign('user',session('member_auth.mid'));
        $this->display();
    }
    ...
    public function get_games() {
        ...
        //推荐游戏
        $reco = $this->get_game_by_type($game_list, 'recommend_status', '1', 3);
        
        //热门游戏
        $hot = $this->get_game_by_type($game_list, 'recommend_status', '2', 6);
    ...
    //换一换
    public function upset_game() {
        $limit = isset($_POST['limit']) && $_POST['limit'] ? $_POST['limit'] : 3;
        $type = isset($_POST['type']) && $_POST['type'] ? $_POST['type'] : 1;

        //推荐游戏
        $reco = M('Game','tab_')->where(array('recommend_status'=>$type,'cover' => array('neq',0)))->limit($limit)->order('rand()')->select();
        if($reco){
            foreach ($reco as $key => $value) {
                $reco[$key]['cover'] = get_cover($value['cover'],'path');
            }
        }
        echo  json_encode($reco);exit;
    }
        ...
        if($user['email'] != $email){
            ...
            $info=M('User_email','tab_')->add($where);
             ...
             echo json_encode(array('status'=>0,'msg'=>'保存信息失败！')); exit;
                  ...
                  $result= sendMail('密码找回','亲爱的手上科技用户您好，您提交了密码找回的申请，点击下方链接进行密码找回，链接24小时内有效:<br/>http://www.moguplay.com/media.php?s=/Index/sendemailfinish/user_id/'.$user['id'].'/create_time/'.$where['create_time'].'/code/'.$where['code'],$where['email']);
                    //file_put_contents('E:/aaa.html',json_encode($result));
                   if($result === true){                        
