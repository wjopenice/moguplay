  ...
  private $_cache;
	 ...
	 */
     public function personalcenter(){
        ...
        if($data['real_name']){
            $data['real_name'] = mb_substr($data['real_name'],1,1,'utf-8')."**";
        }

        if(50 <= $data['levelNum'] &&  $data['levelNum'] < 70){
            $data['level'] = "低";
        }else if (70 <= $data['levelNum'] &&   $data['levelNum'] < 80) {
            $data['level'] = "中";
        }else if (80 <= $data['levelNum']) {
            $data['level'] = "高";
        }
    ...
    public function telsvcode($phone=null,$delay=10,$flag=true) {
        ...
        $re = $xigu->send_verify($phone,$rand);   
        if ($re['Code'] != 'OK') {
            echo json_encode(array('status'=>0,'msg'=>'发送失败，请重新获取'));exit;
        }        
        // 存储短信发送记录信息
        $result['create_time'] = $result['send_time'] = time();
        $result['smsId'] = $re['BizId'];
        $result['pid']=0;
        $result['phone']=$phone;
     ...
     public function tosendemail(){
        ...
        if(!$info){
             echo json_encode(array('status'=>-3,'msg'=>'保存信息失败！')); exit;
              ...
              $in=I('post.in');
              $result= sendMail('邮箱绑定','亲爱的堡庆科技用户您好，您提交了邮箱绑定的申请，点击下方链接进行邮箱绑定，链接24小时内有效:<br/>http://www.moguplay.com/media.php?s=/Member/bindemailfinish/user_id/'.$whereadd['user_id'].'/create_time/'.$whereadd['create_time'].'/code/'.$whereadd['code'].'/in/'.$in,$whereadd['email']);
                //file_put_contents('E:/aaa.html',json_encode($result));
               if($result === true){
     ...
     */
     public function idcard(){
            ...
            if($user['real_name']){
                $user['real_name'] = mb_substr($user['real_name'],1,1,'utf-8')."**";
            }
           $this->assign('user',$user);
           $this->display();
     }
