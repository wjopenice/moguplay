...
class ChargeController extends BaseController {
   ...
   public function agent_pay()
    {
        if(IS_POST){
            ...
            $real_amount=$_POST['amount'];
                ...
                #支付配置
                $config['reqsn'] = $order_no;
                $config["cusid"] = self::CUSID;
                $config["appid"] = self::APPID;
                $config["version"] = self::APIVERSION;
                $config["trxamt"] = $_POST['amount']*100;//$_POST['amount'];
                $config["randomstr"] = $this->getRandom();//
                $config["body"] = "平台币";
                $config["acct"] = "";
                $config["limit_pay"] = "no_credit";
                $config["notify_url"] = "http://" . $_SERVER['HTTP_HOST'] . "/callback.php/NotifyPF/notify";
                switch ($_POST['pay_type']) {
                    case 'alipay':
                        $config["paytype"] = 'A01';
                        $config["remark"] = "支付宝平台币充值";
                        break;
                    case 'weixin':
                        $config["paytype"] = 'W01';
                        $config["remark"] = "微信平台币充值";
                        break;
                    case 'pingtai':
                        $this->add_agent($data);
                        break;
                        ...
                        <div class="text_center">
                            <table class="list" width="100%">
                                ...
                                <tr>
                                    ...
                                    <td class="text_left">本次充值'.$data["amount"].'元，实际付款'.$data["real_amount"].'元</td>
                                </tr>
                                </tbody>
                            </table>
    ...
    */
     ...
     * 支付宝，微信充值，添加代充记录
     * **/
    public function add_pay_agent($data){
        $agent = M("agent","tab_");
