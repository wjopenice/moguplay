...
class IndexController extends BaseController {
    ...
    public function __construct() {
        ...
        $this->assign('user_info',session('member_auth'));
        //初始化
    ...
    public function gift(){
        ...
        foreach($gift as $key=>$value){
            $gift[$key]['url'] = parent::join1_data(
                array(
                    'm_name'=>'Picture',
                    'prefix'=>'sys_',
                    'field' =>'path',
                    'map'   =>array('id'=>$value['icon']),
                )
            );
            if($key > 3){
                unset($gift[$key]);
    ...
    public function verifyvcode() {
        ...
        $account = I('post.account');
        $accountm['user_account']=$account;
        //s$result=$this->step_verify($m,$accountm);
        //file_put_contents('E:/aaa.html',json_encode($result));
        /*if ($result!==true) {
            echo $result;exit;
        }*/
