...
class MemberController extends BaseController {
  ...
  private $_cache;
     ...
     */
     public function mygifts(){
        ...
        $model = array(
            'm_name'=>'record',
            'prefix'=>'tab_gift_',
            'field' =>'tab_giftbag.desribe,tab_gift_record.game_name,tab_gift_record.create_time,tab_giftbag.giftbag_name,tab_giftbag.game_id',
            'join'    =>'tab_giftbag on tab_gift_record.gift_id = tab_giftbag.id',
            'map'   =>array('tab_gift_record.user_id'=>$mid),
            'order' =>'tab_gift_record.create_time desc',
        );
        $gifts = parent::join_data($model);

        if($gifts){
            $game = new GameModel();
            foreach ($gifts as $key=>$value) {
                # 得到游戏图片
                $info = $game->detail($value['game_id']);
                $gifts[$key]['icon'] = $info['icon'];
            }
        }

        $this->assign('count',count($gifts));
    ...
    public function sendvcode() {
        ...
        if (!IS_POST) {
            echo json_encode(array('status'=>0,'msg'=>'非法操作222'));exit;
        }
        $phone = I('phone');
        $name=I('name');
     ...
     public function tosendemail(){
        ...
        if(!$info){
             echo json_encode(array('status'=>-3,'msg'=>'保存信息失败！')); exit;
              ...
              $in=I('post.in');
              $result= sendMail('邮箱绑定','亲爱的手上科技用户您好，您提交了邮箱绑定的申请，点击下方链接进行邮箱绑定，链接24小时内有效:<br/>http://www.game-pk.com/media.php?s=/Member/bindemailfinish/user_id/'.$whereadd['user_id'].'/create_time/'.$whereadd['create_time'].'/code/'.$whereadd['code'].'/in/'.$in,$whereadd['email']);
                //file_put_contents('E:/aaa.html',json_encode($result));
               if($result === true){
    ...
    */
    public function pctrade(){
        ...
        if($type == 0){
            switch ($pay_way) {
                ...
                case 2:
                    //用户直冲
                    $data = $this->platform_user_to_pay($map,$pay_way);
                break;
                case 4:
                    //渠道转移  充值记录        
     ...
     *@author zdd
     */
    public function platform_user_to_pay($map,$way = 0){
