...
<body>
	...
	<div class="cf top_nav_list">
		...
		<?php if(empty($model["extend"])): ?><div class="tools">
                ...
                <button class="btn ajax-post gre" target-form="ids" url="<?php echo U("Promote/set_status",array("status"=>1,"msg_type"=>8,"field"=>"status",'model'=>'AdvPos'));?>">启 用</button>
                <button class="btn ajax-post red" target-form="ids" url="<?php echo U("Promote/set_status",array("status"=>0,"msg_type"=>8,"field"=>"status",'model'=>'AdvPos'));?>">禁 用</button>
			</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
		<div class="search-form fr cf">
            ...
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Adv/media_adv_pos_lists','model='.$model['name'],false);?>">搜索</a>
            </div>
		</div>
	</div>
        ...
        <div class="data-table table-striped">
            <table>
                ...
                <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <a href="<?php echo U('edit_media_adv_pos',array('id'=>$data['id'],'model'=>$model['id']));?>">设置</a>
                            <a href="<?php echo U('add_media_adv',array('model'=>$model['id'],'pos_id'=>$data['id']));?>">添加广告</a>
                        </td>
