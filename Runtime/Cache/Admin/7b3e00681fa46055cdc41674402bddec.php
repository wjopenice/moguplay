		...
		<div class="search-form fr cf">
            ...
            <div class="d_list">
                <div class="drop-down drop-down12" style="width:100px">
                    <span  class="sort-txt" data="{I('type')}" style="width:65px">
                        <?php if(I('type') == ''): ?>操作类型
                        <?php elseif(I('type') == 1): ?>修改密码
                        <?php elseif(I('type') == 2): ?>修改状态
                        <?php elseif(I('type') == 3): ?>修改直冲<?php endif; ?>
                    </span>
                    <i class="arrow arrow-down"></i>
                    <ul  class="nav-list hidden">
                        <li><a href="javascript:;" value="1" style="width:80px">修改密码</a></li>
                        <li><a href="javascript:;" value="2" style="width:80px">修改状态</a></li>
                        <li><a href="javascript:;" value="3" style="width:80px">修改直冲</a></li>
                    </ul>
                </div>
                <input type='hidden' class="hiddenvalue" id='type' name="type" value="<?php echo I('type');?>"></input>
            </div>

			<div class="sleft">
				<input type="text" name="promote_name" class="search-input" value="<?php echo I('promote_name');?>" placeholder="请输入账号">
			</div>
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Promote/promote_change','model='.$model['name'],false);?>">
                搜索
                </a>
            </div>
		</div>
            ...
            <table>
                ...
                <tbody>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo ($data["promote_name"]); ?></td>
                            <td>
                                <?php if($data['type'] == '1'): ?>修改密码
                                <?php elseif($data['type'] == '2'): ?>修改状态
                                <?php else: ?>修改直冲<?php endif; ?>
                            </td>
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Promote/promote_change');?>');
$(function(){
    //批量删除
    $("#dtnn").click(function(){
      var spCodesTemp = "";
