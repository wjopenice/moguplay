...
<body>
	...
	<div class="cf top_nav_list">
		...
		<?php if(empty($model["extend"])): ?><div class="tools">
                ...
                <button class="btn ajax-post gre" target-form="ids" url="<?php echo U("Promote/set_status",array("status"=>1,"msg_type"=>5,"field"=>"status"));?>">启 用</button>
                <button class="btn ajax-post red" target-form="ids" url="<?php echo U("app_adv_pos_lists",array("status"=>2,"msg_type"=>5,"field"=>"status"));?>">禁 用</button>
			</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="<?php echo ((isset($model['search_key']) && ($model['search_key'] !== ""))?($model['search_key']):'title'); ?>" class="search-input" value="<?php echo I('title');?>" placeholder="请输入标题">
				<!-- <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Think/lists','model='.$model['name'],false);?>"><i class="btn-search"></i></a> -->
			</div>
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Adv/app_adv_pos_lists','model='.$model['name'],false);?>">搜索</a>
            </div>
		</div>
	</div>
        ...
        <div class="data-table table-striped">
            <table>
                ...
                <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo get_adv_type($data['type']);?></td>
                        <td><?php echo get_info_status($data['status'],8);?></td>
                        <td><?php echo ($data['width']); ?></td>
                        <td><?php echo ($data['height']); ?></td>
                        <td>
                            <a href="<?php echo U('edit_app_adv_pos',array('id'=>$data['id'],'model'=>$model['id']));?>">设置</a>
                            <a href="<?php echo U('add_app_adv',array('model'=>$model['id'],'pos_id'=>$data['id']));?>">添加广告</a>
                        </td>
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Adv/app_adv_pos_lists');?>');
$(function(){
	//搜索功能
