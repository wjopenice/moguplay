		...
		<div class="search-form fr cf" style="margin-bottom:-15px;">
           <!-- 时间搜索 -->
            ...
            <div class="chose_admin"  id="sel_admin" style="float:left;width:172px;height:28px;line-height:28px;margin:0 5px 0 5px;">
                        ...
                        <input type="hidden"  name="admin_id" value="<?php echo I('admin_id');?>" />        
            </div> 
            ...
            <div class="chose_game"  id="sel_game" style="float:left;width:172px;height:28px;line-height:28px;margin:0 5px 0 5px;">
                        <select>
                            ...
                            <?php $_result=get_game_discount_list();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["game_id"]); ?>"><?php echo ($vo["game_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>                           
                        </select>
            </div> 

            <!-- 推广员搜索 -->
            <div class="sleft">
                <input type="text" name="<?php echo ((isset($model['search_key']) && ($model['search_key'] !== ""))?($model['search_key']):'promote_name'); ?>" class="search-input" value="<?php echo I('promote_name');?>" placeholder="请输入渠道账号">
            </div>
           <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Charge/lists','model='.$model['name'],false);?>">搜索</a>
            </div>
            <!--  导出 -->
            <div class="input-list"> 
                <a class="sch-btn" href="<?php echo U('Export/expUser',array( 'id'=>31, 'admin_id'=>I('admin_id'), 'promote_name'=>I('promote_name'), 'game_id'=>I('game_id'), 'time-start'=>I('time-start'), 'time-end'=>I('time-end'), 'start'=>I('start'), 'end'=>I('end'), ));?>">导出</a>
            </div>
...
<script type="text/javascript">
$(function(){
    // var gameBh="<?php echo ((isset($_GET['game_name']) && ($_GET['game_name'] !== ""))?($_GET['game_name']):0); ?>";game_id
    var gameBh="<?php echo ((isset($_GET['game_id']) && ($_GET['game_id'] !== ""))?($_GET['game_id']):0); ?>";
    var qdzh="<?php echo ($_GET['promote_name']); ?>";
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Charge/lists');?>');
$(function(){
	   $('.ids').ready(function(){
