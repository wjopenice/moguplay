    ...
    <div class="cf top_nav_list">
        ...
        <div class="d_list">
                <div class="drop-down drop-down12" style="width:100px">
                    <span id="sort-txt" class="sort-txt" data="{I('status')}" style="width:65px">
                        <?php if(I('status') == ''): ?>状态
                        <?php elseif(I('status') == 0): ?>未充值
                        <?php elseif(I('status') == 1): ?>已充值<?php endif; ?>
                    </span>
                    <i class="arrow arrow-down"></i>
                    <ul  class="nav-list hidden">
                        <li><a href="javascript:;" value="" style="width:80px">全部</a></li>
                        <li><a href="javascript:;" value="0" style="width:80px">未充值</a></li>
                        <li><a href="javascript:;" value="1" style="width:80px">已充值</a></li>
                    </ul>
                </div>
                <input type='hidden' id='status' name="status" class="hiddenvalue" value="<?php echo I('status');?>"></input>
            </div>
            ...
            <div class="chose_game"  id="sel_game" style="float:left;width:172px;height:28px;line-height:28px;margin:0 5px 0 5px;">
                <select>
                    ...
                    <?php $_result=get_game_list();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(I('game_id') == $vo.id): ?><option value="<?php echo ($vo["id"]); ?>" checked="checked" ><?php echo ($vo["game_name"]); ?></option>
                            <?php else: ?>
                            <option value="<?php echo ($vo["id"]); ?>" ><?php echo ($vo["game_name"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
            <div class="sleft">
                <input type="text" name="<?php echo ((isset($model['search_key1']) && ($model['search_key1'] !== ""))?($model['search_key1']):'pay_order_number'); ?>" class="search-input" value="<?php echo I('pay_order_number');?>" placeholder="请输入订单号" id='pay_order_number'>
            </div>
            ...
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Provide/lists','model='.$model['name'],false);?>">搜索</a>
            </div>
            <div class="input-list">
                <a class="sch-btn" href="<?php echo U('Export/expUser',array('id'=>15,'game_id'=>I('game_id'),'time-start'=>I('time-start'),'time-end'=>I('time-end'),'start'=>I('start'),'end'=>I('end'),));?>">导出</a>
...
$(function(){
  ...
  var spCodesTemp = "";
      ...
      if(spCodesTemp==""){
        ...
        $(this).attr('href','/admin.php?s=/Provide/batch/ids/'+spCodesTemp);
      }
...
$('#time-start').datetimepicker({
  ...
  var spCodesTemp = "";
      ...
      if(spCodesTemp==""){
        ...
        $(this).attr('href','/admin.php?s=/Provide/delprovide/ids/'+spCodesTemp);
      }
