...
<body>
         ...
         <!--渠道 新增搜索功能 2017/8/22 -->
            <div class="chose_game"  id="sel_qd" style="float:left;width:172px;height:28px;line-height:28px;margin:0 5px 0 5px;">
                <select>
                    ...
                    <?php $_result=get_promote_all();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(I('get.promote_id') == $vo.id): ?><option value="<?php echo ($vo["id"]); ?>" checked="checked" ><?php echo ($vo["account"]); ?></option>
                            <?php else: ?>
             ...
             <div class="chose_game"  id="sel_cp" style="float:left;width:172px;height:28px;line-height:28px;margin:0 5px 0 5px;">
              ...
              <div class="sleft" id='pay_style'>
                <div class="drop-down pay_way" style="width: 120px">
                    <span id="sch-pay-way-txt" class="sort-txt" data="<?php echo I('pay_type');?>" style="width: 90px">
                        <?php if(I('pay_type') == ''): ?>充值方式
                        <?php elseif(I('pay_type') == 0): ?>支付宝
                        <?php elseif(I('pay_type') == 2): ?>微信
                ...
                <input type="hidden" id="pay_way" name="pay_type"  value="<?php echo I('pay_type');?>" >
            </div>
            <div id="apply" class="sleft">
                <div  class="drop-down" style="width:95px;" id="pay">
                    <span id="sch-sort-txt" class="sort-txt" style="width:65px;" data="<?php echo I('pay_status');?>">
                        <?php if(I('pay_status') == ''): ?>充值状态
                        <?php elseif(I('pay_status') == 0): ?>充值失败
                        <?php elseif(I('pay_status') == 1): ?>充值成功
                        <?php else: ?>全部<?php endif; ?>
                    </span>
                    <i class="arrow arrow-down"></i>
            ...
            <div class="input-list">
                <a class="sch-btn" href="<?php echo U('Export/expUser',array( 'id'=>1, 'user_account'=>I('user_account'), 'cp_name'=>I('cp_name'), 'game_name'=>I('game_name'), 'game_id'=>I('game_id'), 'promote_id'=>I('promote_id'), 'time-start'=>I('time-start'), 'time-end'=>I('time-end'), 'start'=>I('start'), 'end'=>I('end'), ));?>">导出</a>
                <span class="totalvalue">(共计代充<i><?php echo ($total); ?></i>元)</span>
            </div>
        </div>
    </div>
    ...
    <!-- <span><a href="<?php echo U('Export/expUser',array('id'=>1,'user_account'=>I('user_account')));?>">导出数据(excel格式)</a></span> -->
