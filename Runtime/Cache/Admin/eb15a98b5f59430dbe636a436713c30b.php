        ...
        <div class="search-form fr cf">
            ...
            <div class="i_list">
                 <div class="drop-down" id="promote">
                    <?php if(I('promote_account') == ''): ?><input type="text" id="i_list_id" class="sort-txt" name="promote_account" value="" placeholder="请选择渠道" />
                    <?php else: ?>
                    <input type="text" id="i_list_id" class="sort-txt" name="promote_account" value="<?php echo I('promote_account');?>" placeholder="请选择所属渠道" /><?php endif; ?>
           ...
           <!-- 订单号查询 -->
           <div class="sleft">
                <input type="text" name="<?php echo ((isset($model['search_key']) && ($model['search_key'] !== ""))?($model['search_key']):'pay_order_number'); ?>" class="search-input" value="<?php echo I('pay_order_number');?>" placeholder="请输入订单号">
            ...
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Promote/alipay_list','model='.$model['name'],false);?>">搜索</a>
            </div>
            <div class="input-list">
                <a class="sch-btn" href="<?php echo U('Export/expUser',array( 'id'=>20, 'pay_order_number'=>I('user_account'), 'promote_account'=>I('promote_account'), 'pay_amount'=>I('pay_amount'), 'pay_status'=>I('pay_status'), 'pay_way'=>I('pay_way'), 'create_time'=>I('create_time'), 'time-start'=>I('time-start'), 'time-end'=>I('time-end'), 'start'=>I('start'), 'end'=>I('end'), ));?>">导出</a>
                ...
                <tbody>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td style="border-right:1px solid #DDDDDD;text-align:center">
                                <?php if($data['pay_status'] == 0): ?>充值失败
                                <?php else: ?>充值成功<?php endif; ?>
                            </td>

                            <td style="border-right:1px solid #DDDDDD;text-align:center">
                                <?php if($data['pay_way'] == 1): ?>支付宝
                                <?php elseif($data['pay_way'] == 2): ?>微信
                                <?php elseif($data['pay_way'] == 3): ?>平台币<?php endif; ?>
                            </td>
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Promote/alipay_list');?>');
$(function(){
    //搜索功能
