...
<body>
    ...
    <div class="main-title">
        <h2>[
        <?php if($_GET['category_id']== 49): ?>资讯
        <?php else: ?>活动<?php endif; ?>
        列表]</h2>
    </div>
    <div class="cf top_nav_list">
        <div class="fl button_list">
            <?php if(empty($model["extend"])): ?><div class="tools">
                    <a class="btn blue" href="<?php echo U('APP/app_information_add','category_id='.$_GET['category_id']);?>">新 增</a>
                </div><?php endif; ?>
        </div>
        <!-- 高级搜索 -->
        ...
        <div class="data-table table-striped">
            <table>
                ...
                <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo ($data["title"]); ?></td>
                        <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo ($data["type"]); ?></td>
                        <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo (date("Y-m-d H:i:s",$data["update_time"])); ?></td>
                        <td style="border-right:1px solid #DDDDDD;text-align:center">
                            <?php switch($data["status"]): case "1": ?>推荐<?php break;?>
                               <?php case "2": ?>热门<?php break;?>
                               <?php case "3": ?>最新<?php break;?>
                               <?php default: ?>不推荐<?php endswitch;?> 
                        </td>
                        <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo ($data["admin"]); ?></td>
                        <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo ($data["view"]); ?></td>


                        <td style="border-right:1px solid #DDDDDD;text-align:center">
                            <a href="<?php echo U('app_information_edit',array('ids'=>$data['id'],'category_id'=>$data['category_id']));?>" class="">编辑</a>
    ...
    <script type="text/javascript">
        //导航高亮
        highlight_subnav('<?php echo U('APP/game_type_list');?>');
        $(function(){
            //搜索功能
...
$(function(){
    ...
    $('table tbody').find('tr').each(function(){       
        ...
        $('#delectBtn').find('.sure').click(function(){
            ...
            window.location.href="<?php echo U('app_information_del',array('ids'=>$data['id'],'category_id'=>$data['category_id']));?>";       
        });     
