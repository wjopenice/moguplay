...
<body>
	...
	<div class="cf top_nav_list">
		...
		<?php if(empty($model["extend"])): ?><div class="tools">
				<a class="btn tixian" href="<?php echo U('withdraw_add?model='.$model['id']);?>">提 现</a>
			</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
		<div class="search-form fr cf">
			...
			<div class="sleft">
				<input type="text" name="op_account" class="search-input" value="<?php echo I('op_account');?>" placeholder="请输入操作人账号">
				<!-- <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Query/withdraw','model='.$model['name'],false);?>"><i class="btn-search"></i></a> -->
			</div>
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Query/withdraw','model='.$model['name'],false);?>">搜索</a>
            </div>
            <div class="input-list">
                <a class="sch-btn" href="<?php echo U('Export/expUser',array( 'id'=>6, 'op_account'=>I('op_account'), 'promote_name'=>I('promote_name'), 'time-start'=>I('time-start'), 'time-end'=>I('time-end'), 'start'=>I('start'), 'end'=>I('end'), ));?>">导出</a>
            </div>
		</div>
	</div>
            ...
            <table>
                ...
                <tbody>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo ($data["amount"]); ?></td>
                            <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo ($data["op_account"]); ?></td>
    ...
    <!-- <span><a href="<?php echo U('Export/expUser',array( 'id'=>6, 'op_account'=>I('op_account'), 'time-start'=>I('time-start'), 'time-end'=>I('time-end'), 'start'=>I('start'), 'end'=>I('end'), ));?>">导出数据(excel格式)</a></span> -->
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Query/withdraw');?>');
$(function(){
	//搜索功能
