...
<body>
	...
	<form action="<?php echo U('save');?>" method="post" class="form-horizontal">
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i; switch($config["name"]): case "CH_SET_LOGO": ?><div class="form-item">
				...
				<?php case "CH_SET_QRCODE": ?><div class="form-item">
						<label class="item-label"><?php echo ($config["title"]); ?><span class="check-tips">（<?php echo ($config["remark"]); ?>）</span> </label>
						<div class="controls">
				...
				<?php case "CH_SET_ICO": ?><div class="form-item">
						<label class="item-label"><?php echo ($config["title"]); ?><span class="check-tips">（<?php echo ($config["remark"]); ?>）</span> </label>
						<div class="controls">
...
<script type="text/javascript">
	//导航高亮
	highlight_subnav('<?php echo U('Lgog/channel_logo');?>');
