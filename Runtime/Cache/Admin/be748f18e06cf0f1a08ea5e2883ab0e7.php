...
<body>
    ...
    <form action="<?php echo U('version_edit');?>" method="post" class="form-horizontal tab-list" id='form01'>
    <!-- 版本管理的编辑id隐藏域 -->
        <input type="hidden" name="id" value="<?php echo ($_GET['id']); ?>">
        <div class="form-item">
            <label class="item-label">服务器端版本号<span class="check-tips">（升级至此版本）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="serverversion" value="<?php echo ($data["serverversion"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">app版本名<span class="check-tips">（说明）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="appname" value="<?php echo ($data["appname"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">服务器标识</label>
            <div class="controls">
                <input type="text" class="text input-large" name="serverflag" value="<?php echo ($data["serverflag"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">lastForce<span class="check-tips"></span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="lastforce" value="<?php echo ($data["lastforce"]); ?>">
            </div>
        </div>
        <div class="form-item">
             <label class="item-label">更新说明<span class="check-tips">（说明）</span></label>
            <div class="controls">               
                <label class="textarea input-large">
                    <textarea name="upgradeinfo"><?php echo ($data["upgradeinfo"]); ?></textarea>
                </label>
            </div>
        </div>
        <div class="form-item">
             <label class="item-label">更新状态<span class="check-tips">（说明）</span></label>
            <select name="is_must">
            <?php if($data["is_must"] == 1 ): ?><option value="0" >不强制更新</option>
                <option value="1" selected>强制更新</option>
            <?php else: ?>
            	<option value="0" selected>不强制更新</option>
                <option value="1">强制更新</option><?php endif; ?>
                    
            </select>
        </div>
        <div class="form-item">
             <input type="submit" name="submit" value="确 定" style="border:none;width:70px;height: 40px;color:#fff; background-color: #2062b0;   margin: 30px 10px;padding: 10px;line-height: 20px; cursor: pointer;" id='sub-btn'>
    ...
    <script type="text/javascript">
        //导航高亮
        highlight_subnav('<?php echo U('APP/version_edit');?>');
        $(function(){
          //  function kong(){
          //   var serverVersion=$.trim($('input[name=serverversion]').val());
          //   if(serverVersion==''){
          //       alert('请输入服务器端版本号')
          //      return false;
          //   } 
          //   var reg=/^\d+$/g;
          //   if(!reg.test(serverVersion)){
          //       alert('请输入正确的服务器端版本号')
          //       return false;
          //   } 
          // } 
