...
<body>
	...
	<div class="main-title"><h2>网站设置</h2></div>
	<div class="tab-wrap">
		...
		<div class="tab-content">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i; switch($config["name"]): case "PC_SET_LOGO": ?><div class="form-item">
						...
						<div class="controls">
			                ...
			                <input type="hidden" name="config[<?php echo ($config["name"]); ?>]" id="cover_id_logo" value="<?php echo ($config["value"]); ?>" />
			                <div class="upload-img-box">
			                <?php if(!empty($config['value'])): ?><div class="upload-pre-item"><img src="<?php echo (get_cover($config['value'],'path')); ?>"/></div><?php endif; ?>
			                </div>
                            <!-- <span class="infonotice" style="line-height:40px;" >(190*60px)</span> -->
						</div>
					</div><?php break;?>
					<?php case "PC_SET_QRCODE": ?><div class="form-item">
							<label class="item-label"><?php echo ($config["title"]); ?><span class="check-tips">（<?php echo ($config["remark"]); ?>）</span> </label>
							<div class="controls">
								<input type="file" id="upload_picture_qrcode">
				                <input type="hidden" name="config[<?php echo ($config["name"]); ?>]" id="cover_id_qrcode"/>
				                <div class="upload-img-box">
				                <?php if(!empty($config['value'])): ?><div class="upload-pre-item"><img src="<?php echo (get_cover($config['value'],'path')); ?>"/></div><?php endif; ?>
				                </div>
					...
					<?php case "PC_SET_ICO": ?><div class="form-item">
							<label class="item-label"><?php echo ($config["title"]); ?><span class="check-tips">（<?php echo ($config["remark"]); ?>）</span> </label>
							<div class="controls">
								<input type="file" id="upload_picture_ico">
				                <input type="hidden" name="config[<?php echo ($config["name"]); ?>]" id="cover_id_ico"/>
				                <div class="upload-img-box">
				                <?php if(!empty($config['value'])): ?><div class="upload-pre-item"><img src="<?php echo (get_cover($config['value'],'path')); ?>"/></div><?php endif; ?>
				                </div>
...
<script type="text/javascript">
	//导航高亮
	highlight_subnav('<?php echo U('Lgog/media_logo');?>');
	...
	$("#upload_picture_logo").uploadify({
	    ...
	    "onUploadSuccess" : upload_picture_logo<?php echo ($field["name"]); ?>,
	    'onFallback' : function() {
	        alert('未检测到兼容版本的Flash.');
	    }
	});
	function upload_picture_logo<?php echo ($field["name"]); ?>(file, data){
	...
	$("#upload_picture_qrcode").uploadify({
	    ...
	    'removeTimeout'   : 1,
	    'fileTypeExts'    : '*.jpg; *.png; *.gif;',
	    "onUploadSuccess" : upload_picture_qrcode<?php echo ($field["name"]); ?>,
	    'onFallback' : function() {
	        alert('未检测到兼容版本的Flash.');
	    }
	});
	function upload_picture_qrcode<?php echo ($field["name"]); ?>(file, data){
	    var data = $.parseJSON(data);
	    var src = '';
	    if(data.status){
	...
	$("#upload_picture_ico").uploadify({
	    ...
	    'removeTimeout'   : 1,
	    'fileTypeExts'    : '*.ico;',
	    "onUploadSuccess" : upload_picture_ico<?php echo ($field["name"]); ?>,
	    'onFallback' : function() {
	        alert('未检测到兼容版本的Flash.');
	    }
	});
	function upload_picture_ico<?php echo ($field["name"]); ?>(file, data){
	    var data = $.parseJSON(data);
	    var src = '';
	    if(data.status){
