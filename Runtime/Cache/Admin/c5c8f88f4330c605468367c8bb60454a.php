...
<body>
    ...
    <form action="<?php echo U('edit',array('id'=>I('id')));?>" onsubmit = "check();" method="post" class="form-horizontal tab-list">
        <div class="form-item">
            <label class="item-label">用户名<span class="check-tips">（用户名会作为默认的昵称）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" value="<?php echo ($list["username"]); ?>" disabled="disabled">
                <input type="hidden" class="text input-large" name="username" value="<?php echo ($list["username"]); ?>" >
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">密码<span class="check-tips">（用户密码不能少于6位）</span></label>
            <div class="controls">
                <input type="password" class="text input-large" id="password" name="password" value="<?php echo ($list["password"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">邮箱<span class="check-tips">（用户邮箱，用于找回密码等安全操作）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="email" value="<?php echo ($list["email"]); ?>">
            </div>
        </div>
        <div class="form-item">
            ...
            <select name="auth" id="authid">
                <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sd): $mod = ($i % 2 );++$i;?><option value="<?php echo ($sd['id']); ?>"><?php echo ($sd['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
        <div class="form-item">
             <label class="item-label">所属后台<span class="check-tips">（管理员可以访问的后台）</span></label>
            <select name="houtai">
            <option value="0"<?php if(($houtai) == "0"): ?>selected='selected'<?php endif; ?>>总后台</option>
                    <option value="1" <?php if(($houtai) == "1"): ?>selected='selected'<?php endif; ?>>商务后台</option>
            </select>
        </div>
        <div class="form-item">
    ...
    <script type="text/javascript">
        Think.setValue("auth", <?php echo ((isset($authid) && ($authid !== ""))?($authid): 0); ?>);
