...
<body>
    ...
    <div class="cf top_nav_list">
        ...
        <?php if(empty($model["extend"])): ?><div class="tools">
                 <a class="btn blue" href="<?php echo U('pay_limit_add?model='.$model['id']);?>">新 增</a>
                <!-- <button class="btn ajax-post confirm" target-form="ids" url="<?php echo U('pay_limit_del?model='.$model['id']);?>">删 除</button> -->
            </div><?php endif; ?>
        </div>
        <!-- 高级搜索 -->
        <div class="search-form fr cf">
            ...
            <div class="chose_admin"  id="sel_admin" style="float:left;width:172px;height:28px;line-height:28px;margin:0 5px 0 5px;">
                        <select>
                            <?php if(I('nickname') == ''): ?><option value="" >请选择管理员</option>
                            <?php else: ?>
                                <option value="<?php echo I('nickname');?>">请选择管理员</option><?php endif; ?>                                                     
                            <option value="">全部管理员</option>                            
                            <?php $_result=get_member_list();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["uid"]); ?>"><?php echo ($vo["nickname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>                           
                        </select>
            </div> 
            ...
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Promote/pay_limit','model='.$model['name'],false);?>">搜索</a>
            </div>
            <div class="input-list">
                <a class="sch-btn" href="<?php echo U('Export/expUser',array( 'id'=>13, 'account'=>I('account'), 'promote_name'=>I('promote_name'), 'time-start'=>I('time-start'), 'time-end'=>I('time-end'), 'start'=>I('start'), 'end'=>I('end'), ));?>">导出</a>
            </div>
        </div>
    </div>
            ...
            <table>
                ...
                <tbody>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo ($data["pay_limit"]); ?></td>
                            <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo get_admin_nickname($data['admin_id']);?></td>
                            <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo set_show_time($data['set_pay_time']);?></td>
                            <td style="border-right:1px solid #DDDDDD;text-align:center">
                            <a href="<?php echo U('pay_limit_edit',array('ids'=>$data['id']));?>" class="">编辑</a>
                            <a href="<?php echo U('pay_limit_del',array('ids'=>$data['id']));?>" class="">删除</a>
                            </td>
    ...
    <!-- <span><a href="<?php echo U('Export/expUser',array( 'id'=>3, 'account'=>I('account'), 'time-start'=>I('time-start'), 'time-end'=>I('time-end'), 'start'=>I('start'), 'end'=>I('end'), ));?>">导出数据(excel格式)</a></span> -->
...
<script type="text/javascript">
$(function(){    
    var choseAdmin="<?php echo ($_GET['admin_id']); ?>";  
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Promote/pay_limit');?>');
$(function(){
    //搜索功能
    $("#search").click(function(){
        var url = $(this).attr('url');
        // var query  = $('#promote').find('input').serialize();        
        var query  = $.param({           
            'admin_id':$("#sel_admin select").val(),
            'promote_name':$('#promote').find('input').val()
        });
