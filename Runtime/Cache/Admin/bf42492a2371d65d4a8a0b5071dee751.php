...
<body>
	...
	<div class="cf top_nav_list">
		...
		<?php if(empty($model["extend"])): ?><div class="tools">
				<a class="btn blue" href="<?php echo U('exchange_add');?>">新 增</a>
<!--                 <button class="btn ajax-post confirm red" target-form="ids" url="<?php echo U('Giftbag/exchange_del?id='.$vo['id']);?>">删 除</button> -->
			</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
		<div class="search-form fr cf">
			...
			<div class="sleft">
				<input type="text" name="<?php echo ((isset($model['search_key']) && ($model['search_key'] !== ""))?($model['search_key']):'title'); ?>" class="search-input" value="<?php echo I('title');?>" placeholder="请输入激活码名称">
				<!-- <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Giftbag/lists','model='.$model['name'],false);?>"><i class="btn-search"></i></a> -->
			</div>
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Giftbag/exchange_list','model='.$model['name'],false);?>">搜素</a>
            </div>
		</div>
	</div>
   ...
   <!-- 数据列表 -->
    ...
    <tbody>
        <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            ...
            <td><?php echo (arr_count($vo["novice"])); ?></td>
            <td>
                 <?php if($vo['status'] == 0): ?>关闭
                        <?php elseif($vo['status'] == 1): ?>开启
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Giftbag/exchange_list');?>');
$(function(){
	//搜索功能
