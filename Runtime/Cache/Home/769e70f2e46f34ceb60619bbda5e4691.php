...
<div class="bgWhite moduleA">
  <div class="section clear">
    <div class="main mainGame">
      <div class="titA clear">
        <h3 class="tname" id="mainGameTab"><span class="setTab-tab on">热门手游推荐</span></h3>
        <a href="<?php echo U('Article/game_list');?>" class="more">更多+</a> </div>
      <div class="gameList" id="mainGameList">
        
        <ul class="setTab-panel clear">
          <?php if(is_array($game_list)): $i = 0; $__LIST__ = $game_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
      			<div class="sp1 fl">
      				<a href="<?php if($vo["dow_status"] == 0): ?>#<?php else: echo U('Down/media_down_file?game_id='.$vo['id'].'&type=1'); endif; ?>" target="_blank"><img src="<?php echo (get_cover($vo["icon"],'path')); ?>"></a>
      			</div>
      			<div class="sp2 fl">
      				<h3><a href="<?php if($vo["dow_status"] == 0): ?>#<?php else: echo U('Down/down_file?game_id='.$vo['id'].'&type=1'); endif; ?>" target="_blank"><?php echo ($vo["game_name"]); ?></a></h3>
      				<p class="gamestyle"><span><?php echo ($vo['game_type_name']); ?></span></p>
      				<div class="gamedown"><a href="<?php if($vo["dow_status"] == 0): ?>#<?php else: echo U('Down/media_down_file?game_id='.$vo['id'].'&type=1'); endif; ?>" class="down">下载</a></div>
      			</div>
    ...
    <div class="sideBar">
      <div class="newsPanel fl">
        <div class="titA clear">
          <h3 class="tname"><span class="on">公告</span></h3>
          <a href="<?php echo U('Article/lists?category=tui_gg');?>" target="_blank" class="more">更多+</a> </div>
        <div class="newslist">
          <ul class="txtlist">
      		   <?php $__CATE__ = D('Category')->getChildrenId(2);$__LIST__ = D('Document')->lists_limit($__CATE__, '`level` DESC,`id` DESC', 1,true,5); if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><li>
                <span class="fr"><?php echo (date('Y-m-d',$list["create_time"])); ?></span>
                <a href="<?php echo U('Article/detail?id='.$list['id']);?>" target="_blank">【公告】<?php echo ($list["title"]); ?></a>
              </li><?php endforeach; endif; else: echo "" ;endif; ?>
          </ul>
        </div>
      </div>
      <div class="kaifu fr">
        <div class="titA clear">
          <h3 class="tname"><span class="on">资讯</span></h3>
          <a href="<?php echo U('Article/lists?category=tui_zx');?>" target="_blank" class="more">更多+</a> 
        </div>
        <ul class="txtlist">
